...
import { UIContext } from '../vector/retrieveContext.js';
...
export async function generateTestCases(context: UIContext, featureName: string): Promise<string> {
  ...
  const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' }); // Use a more standard model name if needed, or keep as is
